 ...
 *
 * Thermoelectric generator capturing energy from temperature differentials.
 * Typical output: 5–20W where ΔT exists.
 */
...
export class ThermalHarvester implements IEnergyHarvester {
  ...
  constructor(
    ...
    this.temperature = celsius(35);
  }

  getSourceType(): EnergySourceType {
    return EnergySourceType.THERMAL;
  }
  ...
  startHarvesting(): HarvestResult {
    if (this.status === HarvesterStatus.FAULT) {
      return {
        success: false,
        sourceType: EnergySourceType.THERMAL,
    ...
    if (this.availability === SourceAvailability.NONE) {
      ...
      return {
        success: false,
        sourceType: EnergySourceType.THERMAL,
        error: "No thermal gradient available",
      };
    }
    ...
    return { success: true, sourceType: EnergySourceType.THERMAL };
  }
